...
\begin{itemize}
    \item[-] Nếu checksum UDP được tính là incorrect → gói tin bị coi là corrupted, UDP handler ở phía nhận sẽ ngay lập tức loại bỏ gói tin đó và không chuyển payload lên tầng ứng dụng (DHCP).
    \item[-] DHCP stall vì client không nhận Offer hoặc ACK: DHCP dựa vào chuỗi 4 gói Discover – Offer – Request – ACK. Nếu một gói bị drop thì quá trình sẽ không thể tiếp tục và chờ timeout.:
    \begin{itemize}
        \item[$\bullet$] Client không nhận Offer → server không thấy client → phải timeout và gửi Discover lại.
        \item[$\bullet$] DHCP Offer bị drop → client không biết có server
        \item[$\bullet$] Server không nhận Request → không cấp IP
        \item[$\bullet$] DHCP ACK bị drop → client không hoàn tất cấp IP
    \end{itemize}
    \item[-] Trong khoảng thời gian đó, DHCP bị stall.
\end{itemize}
