...
\begin{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.8\textwidth]{img/2-cau1-1.png}
...
arp && frame.time_relative >= 14.265823 && frame.time_relative <= 14.896409\end{lstlisting}
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.8\textwidth]{img/2-cau1-2.png}
...
arp && frame.time_relative >= 34.055350 && frame.time_relative <= 34.579622\end{lstlisting}
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.8\textwidth]{img/2-cau1-3.png}
...
\begin{table}[H]
    \centering
    \begin{tabular}{|l|l|p{3.5cm}|p{6cm}|}
        \hline
        \textbf{DHCP Message} & \textbf{Source IP} & \textbf{Destination IP} & \textbf{Giải thích} \\
        \hline
        DHCP Discover & 0.0.0.0 & 255.255.255.255 & Client chưa có IP nên dùng địa chỉ không xác định (0.0.0.0) và gửi broadcast để tìm DHCP server. \\
        \hline
        DHCP Offer & IP của DHCP server & 255.255.255.255 & Server trả lời bằng broadcast vì client chưa thể nhận unicast. \\
        \hline
        DHCP Request & 0.0.0.0 & 255.255.255.255 & Client vẫn chưa được cấp IP chính thức, gửi broadcast để chấp nhận địa chỉ đề nghị. \\
        \hline
        DHCP ACK & IP của DHCP server & 255.255.255.255 (hoặc unicast) & Server xác nhận cấp IP; client chỉ sử dụng IP sau khi nhận ACK. \\
        \hline
    \end{tabular}
...
\begin{itemize}
    ...
    \item[-] Vì vậy, trong DHCP Discover và DHCP Request, client không thể dùng IP riêng và bắt buộc dùng source IP = 0.0.0.0
    \item[-] Các gói DHCP được gửi dưới dạng broadcast (255.255.255.255) để đảm bảo DHCP server nhận được và client có thể nhận phản hồi dù chưa có IP hợp lệ
...
\begin{itemize}
    \item[-] Client phải hủy IP vừa được cấp (decline) và gửi DHCPDECLINE cho server.
    \item[-] Theo RFC 2131:
    \begin{itemize}
...
\begin{itemize}
    \item[-] UDP là connectionless → không cần thiết lập phiên: DHCP hoạt động ngay cả khi host chưa có IP, chưa có routing, chưa có state. Port cố định đảm bảo mọi thiết bị đều biết nơi gửi và nhận frame.
    \item[-] Chuẩn hóa để tất cả DHCP server/relay có thể hoạt động thống nhất
    \begin{itemize}
        \item[$\bullet$] Client luôn gửi từ port 68
        \item[$\bullet$] Server luôn lắng nghe tại port 67
    \end{itemize}
    \item[-] Relay Agent (DHCP Relay) cần port cố định: Các router chuyển tiếp DHCP cần biết chính xác port để forward.
    \item[-] Loại bỏ lỗi tương thích: Nếu dùng port ngẫu nhiên → server không biết nên gửi ACK về đâu.
\end{itemize}
...
\begin{itemize}
    \item[-] Trong Windows và Linux, Wireshark thường hiển thị:
"[UDP checksum is good]"
hoặc
"Checksum validation disabled for UDP" (tùy OS checksum offloading)
Dựa trên screenshot của bạn, gói không có lỗi → checksum GOOD.
    \item[-] 
\end{itemize}
...
\begin{itemize}
    \item[-] UDP handler sẽ:
    \begin{itemize}
        \item[$\bullet$] Loại bỏ gói (drop silently): UDP không tự sửa lỗi, không retransmission. Nó chỉ kiểm tra checksum → nếu sai → discard.
        \item[$\bullet$] DHCP stall vì client không nhận Offer hoặc ACK: DHCP dựa vào chuỗi 4 gói BOOTP (Discover–Offer–Request–ACK). Nếu một gói bị drop:
        \begin{itemize}
            \item[$\circ$] Client không nhận Offer → phải timeout và gửi Discover lại.
            \item[$\circ$] Server không nhận Request → không cấp IP
            \item[$\circ$] Toàn bộ chu trình DHCP dừng hoặc timeout (30–60 giây)
        \end{itemize}
        \item[$\bullet$] DHCP không có cơ chế “retry nhanh” vì UDP không đảm bảo delivery.
    \end{itemize}
\end{itemize}
